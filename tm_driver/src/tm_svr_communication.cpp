...
bool TmSvrCommunication::start_tm_svr(int timeout_ms)
{
  ...
  const bool rb = connect_socket("Ethernet slave communication", timeout_ms);
...
{
  const TmSvrData cmd{id, mode, content.data(), content.size(), TmSvrData::SrcType::Shallow};
  TmPacket pack{cmd};
...
{
  const TmSvrData cmd{id, TmSvrData::Mode::STRING, content.data(), content.size(), TmSvrData::SrcType::Shallow};
  TmPacket pack{cmd};
  return send_packet_all(pack);
}
...
{
  print_debug("Ethernet slave communication: thread begin");
    ...
    while (_keep_thread_alive && is_connected() && !reconnect)
    {
      ...
      switch (rc)
      {
        ...
        case TmCommRC::TIMEOUT:
          print_debug("Ethernet slave communication: rc=%d", static_cast<int>(rc));
          reconnect = true;
          break;
        default:
          break;
  ...
  close_socket();
  _cv->notify_all();
  print_debug("Ethernet slave communication: thread end");
}
      ...
      if (data.is_valid())
      {
        switch (data.mode())
        {
          case TmSvrData::Mode::RESPONSE:
            print_debug("Ethernet slave communication: RESPONSE (%s): [%d]: %s", data.transaction_id().c_str(),
          ...
          case TmSvrData::Mode::READ_STRING:
            print_debug("Ethernet slave communication: READ_STRING (%s): %s", data.transaction_id().c_str(),
                        std::string(data.content(), data.content_len()).c_str());
            break;
          default:
            print_error("Ethernet slave communication: (%s): invalid mode (%d)", data.transaction_id().c_str(),
                        (int)(data.mode()));
            break;
        }
      }
