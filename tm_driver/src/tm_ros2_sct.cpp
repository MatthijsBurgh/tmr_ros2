...
const std::array<std::string, 6> TmSctRos2::jns_ = {"joint_1", "joint_2", "joint_3", "joint_4", "joint_5", "joint_6"};

TmSctRos2::TmSctRos2(const rclcpp::Node::SharedPtr& node, TmDriver& iface, bool is_fake)
  : node(node), iface_(iface), is_fake_(is_fake)
{
  if (!is_fake_)
  {
    listenNodeConnection = std::make_unique<ListenNodeConnection>(
        iface_, std::bind(&TmSctRos2::sct_msg, this, std::placeholders::_1),
        std::bind(&TmSctRos2::sta_msg, this, std::placeholders::_1, std::placeholders::_2), is_fake_);
...
TmSctRos2::~TmSctRos2()
{
  print_debug("TmSctRos2::~TmSctRos2");
...
void TmSctRos2::sct_msg(TmSctData data)
{
  ...
  if (data.sct_has_error())
  {
    ...
    print_error("TM_ROS: (TM_SCT): ROS Node Data Error %d", static_cast<int>(data.sct_has_error()));
  }
  else
  {
...
void TmSctRos2::sta_msg(const std::string& subcmd, const std::string& subdata)
{
  SctAndStaMsg& sm = sm_;
...
bool TmSctRos2::send_script(
    ...
    std::shared_ptr<tm_msgs::srv::SendScript::Response> res)  // NOLINT(performance-unnecessary-value-param)
{
  res->ok = listenNodeConnection->send_listen_node_script(req->id, req->script);
  return res->ok;
}

bool TmSctRos2::set_event(
    const std::shared_ptr<tm_msgs::srv::SetEvent::Request>& req,
    std::shared_ptr<tm_msgs::srv::SetEvent::Response> res)  // NOLINT(performance-unnecessary-value-param)
{
  res->ok = false;
  switch (req->func)
  {
    case tm_msgs::srv::SetEvent_Request::EXIT:
      res->ok = iface_.script_exit();
      break;
    case tm_msgs::srv::SetEvent_Request::TAG:
      res->ok = iface_.set_tag(static_cast<int>(req->arg0), static_cast<int>(req->arg1));
      break;
    case tm_msgs::srv::SetEvent_Request::WAIT_TAG:
      res->ok = iface_.set_wait_tag(static_cast<int>(req->arg0), static_cast<int>(req->arg1));
      break;
    case tm_msgs::srv::SetEvent_Request::STOP:
      res->ok = iface_.set_stop();
      break;
    case tm_msgs::srv::SetEvent_Request::PAUSE:
      res->ok = iface_.set_pause();
      break;
    case tm_msgs::srv::SetEvent_Request::RESUME:
      res->ok = iface_.set_resume();
      break;
  }
  return res->ok;
}

bool TmSctRos2::set_io(const std::shared_ptr<tm_msgs::srv::SetIO::Request>& req,
                       std::shared_ptr<tm_msgs::srv::SetIO::Response> res)  // NOLINT(performance-unnecessary-value-param)
{
  res->ok = iface_.set_io(TmIOModule(req->module), TmIOType(req->type), int(req->pin), req->state);
  return res->ok;
}

bool TmSctRos2::set_positions(
    const std::shared_ptr<tm_msgs::srv::SetPositions::Request>& req,
    std::shared_ptr<tm_msgs::srv::SetPositions::Response> res)  // NOLINT(performance-unnecessary-value-param)
{
  res->ok = false;
  switch (req->motion_type)
  {
    case tm_msgs::srv::SetPositions_Request::PTP_J:
      res->ok =
          iface_.set_joint_pos_PTP(req->positions, req->velocity, req->acc_time, req->blend_percentage, req->fine_goal);
      break;
    case tm_msgs::srv::SetPositions_Request::PTP_T:
      res->ok =
          iface_.set_tool_pose_PTP(req->positions, req->velocity, req->acc_time, req->blend_percentage, req->fine_goal);
      break;
    case tm_msgs::srv::SetPositions_Request::LINE_T:
      res->ok = iface_.set_tool_pose_Line(req->positions, req->velocity, req->acc_time, req->blend_percentage,
                                          req->fine_goal);
      break;
  }
  return res->ok;
...
bool TmSctRos2::ask_sta(
    ...
    std::shared_ptr<tm_msgs::srv::AskSta::Response> res)  // NOLINT(performance-unnecessary-value-param)
{
