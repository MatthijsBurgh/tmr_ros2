...
void TmPacket::build_bytes(std::vector<char>& bytes, const TmPacket& packet)
{
  ...
  // Length
  std::string slen = std::to_string(packet.data.size());
...
void TmPacket::build_bytes(std::string& bytes, const TmPacket& packet)
{
  ...
  bytes.push_back(P_SEPR);
  // Length
  bytes.append(std::to_string(packet.data.size()));
  bytes.push_back(P_SEPR);
  // Data
  bytes.insert(bytes.end(), std::begin(packet.data), std::end(packet.data));
  bytes.push_back(P_SEPR);
...
size_t TmPacket::build_packet_from_bytes(TmPacket& packet, const char* bytes, size_t size)
{
  ...
  if (ind_e > 1)
  {
    const std::string hdr{bytes + ind_b, ind_e - ind_b};
  ...
  if (ind_e > ind_b)
  {
    const std::string len{bytes + ind_b, ind_e - ind_b};
    length = std::stoi(len);
  }
  ++ind_e;
  ind_b = ind_e;
...
{
  std::array<char, 3> cc = {buf[0], buf[1], '\0'};
  const int ic = std::atoi(cc.data());
...
void TmSvrData::build_TmSvrData(TmSvrData& data, const std::string& id, Mode mode, const char* content, size_t len,
                                SrcType type)
{
  data._is_copy = (type != SrcType::Shallow);
...
void TmSvrData::build_TmSvrData(TmSvrData& data, const char* bytes, size_t size, SrcType type)
{
  ...
  std::array<char, 3> amode = {bytes[ind_e], '\0', '\0'};
  char cmode = 0;
  ...
  cmode = std::atoi(amode.data());
  if (cmode > static_cast<char>(Mode::UNKNOW))
  {
    return;
  }
  ++ind_e;
...
void TmSvrData::build_bytes(std::vector<char>& bytes, const TmSvrData& data)
{
  ...
  bytes.push_back(TmPacket::P_SEPR);
  const size_t ind_b = bytes.size();
  size_t ind_e = ind_b;
  const size_t new_size = ind_e + data._len;
...
void FakeTmSvrPacket::build_content(std::vector<char>& content, const std::array<float, 6>& angle,
                                    const std::array<float, 6>& pose)
{
  ...
  content.push_back(ulen.ca[0]);
  content.push_back(ulen.ca[1]);
  std::array<char, 24> fdata;
  memcpy(fdata.data(), angle.data(), 24);
  for (int i = 0; i < 24; ++i)
  {
  ...
  content.push_back(ulen.ca[0]);
  content.push_back(ulen.ca[1]);
  memcpy(fdata.data(), pose.data(), 24);
  for (int i = 0; i < 24; ++i)
  {
    content.push_back(fdata[i]);
  }
...
void TmSctData::build_bytes(std::vector<char>& bytes, const TmSctData& data)
{
  ...
  bytes.push_back(TmPacket::P_SEPR);
  const size_t ind_b = bytes.size();
  const size_t new_size = ind_b + data._len;
  bytes.resize(new_size);
  for (size_t ind_e = ind_b; ind_e < new_size; ++ind_e)
  {
...
void TmStaData::build_TmStaData(TmStaData& data, const std::string& sub_cmd, const char* sub_data, size_t len,
                                SrcType type)
{
  data._is_copy = (type != SrcType::Shallow);
  data._is_valid = false;
