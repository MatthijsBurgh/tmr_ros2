...
EthernetSlaveConnection::EthernetSlaveConnection(TmDriver& iface, const std::function<void(void)>& ethernet_cmd_come,
                                                 bool stick_play)
  : svr_(iface.svr), state_(iface.state), sct_(iface.sct), iface_(iface), publish_svr(ethernet_cmd_come)
{
  const bool rb = iface_.svr.start_tm_svr(5000);
  if (rb && stick_play)
  {
...
bool EthernetSlaveConnection::get_data_function()
{
  ...
  bool has_complete_package = false;
  for (auto& pack : pack_vec)
  {
      ...
      if (svr.data.is_valid())
      {
        switch (svr.data.mode())
        {
          case TmSvrData::Mode::RESPONSE:
            print_debug("TM_ROS: (Ethernet slave): (%s) RESPONSE [%d]", svr.data.transaction_id().c_str(),
                        (int)(svr.data.error_code()));
            publish_svr();
            break;
          ...
          case TmSvrData::Mode::READ_JSON:
            ...
            break;
          default:
            print_error("TM_ROS: (Ethernet slave): (%s): invalid mode (%d)", svr.data.transaction_id().c_str(),
                        (int)(svr.data.mode()));
            break;
        }
      }
    ...
    else if (pack.type == TmPacket::Header::EMPTY)
    {
      ...
      {
        print_warn("warnning TM_ROS: (Ethernet slave): receive package header may not complete, but some data formate "
                   "is correct, please check your network");
      }
      else
      {
    ...
    else if (pack.type == TmPacket::Header::PACKAGE_INCOMPLETE)
    {
      if (has_complete_package)
      {
        print_warn("warnning TM_ROS: (Ethernet slave): receive package is not complete, but some data formate is "
                   "correct, please check your network");
      }
      else
      {
...
void EthernetSlaveConnection::svr_connect_recover()
{
  ...
  while (run && time_interval < pub_reconnect_timeval_ms_)
  {
    if (last_time_interval / 1000 != time_interval / 1000)
    {
      print_debug("Ethernet slave reconnect remain : %.1f sec...", 0.001 * (pub_reconnect_timeval_ms_ - time_interval));
    }
...
bool EthernetSlaveConnection::rc_halt()
{  // Stop rescue connection
  ...
  {
    print_debug("TM_ROS: (Ethernet slave): notConnectTimeInS = %d, maxTrialTimeInMinute = %d ", (int)notConnectTimeInS,
                (int)maxTrialTimeInMinute);
    return true;
  }
  return false;
}

void EthernetSlaveConnection::cq_monitor()
{  // Connection quality
  diconnectTimes++;
...
void EthernetSlaveConnection::get_data_thread()
{
  ...
  print_debug("TM_ROS: get data thread begin");
  initialNotConnectTime = TmCommunication::get_current_time_in_ms();
  while (run)
  {
    ...
    else
    {
      if (!svr.recv_init())
      {
        ...
        if (rc_halt())
        {
          ...
          {
            print_fatal("TM_ROS: (Ethernet slave): LinkLost = %d , MaxLostTime(s) = %d", diconnectTimes,
                        maxNotConnectTimeInS);
          }
...
bool EthernetSlaveConnection::re_connect(int timeout_ms, int timeval_ms)
{
  ...
  print_info("TM_ROS: set Ethernet slave reconnect timeout %dms, timeval %dms", (int)pub_reconnect_timeout_ms_,
