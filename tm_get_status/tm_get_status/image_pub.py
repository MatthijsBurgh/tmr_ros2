...
class ImagePub(Node):
    def __init__(self, node_name: str, is_test: bool, path: str) -> None:
    ...
    def set_image_and_notify_send(self, img: np.ndarray) -> None:
        self.con.acquire()
    ...
    def publish_test_image(self) -> None:
        self.img = cv2.flip(self.img, 1)
        self.set_image_and_notify_send(self.img)

    def image_publisher(self, image: np.ndarray) -> None:
        bridge = CvBridge()
    ...
    def pub_data_thread(self, is_request_data: bool) -> None:
        self.con.acquire()
        while rclpy.ok():
            self.con.wait()
    ...
    @staticmethod
    def fake_result(m_method: str) -> Mapping[str, Optional[Union[float, str]]]:
        # classification
        if m_method == "CLS":
            # inference img here
            result = {"message": "success", "result": "NG", "score": 0.987}
        # detection
        elif m_method == "DET":
            # inference img here
            result = {
        ...
        # no method
            ...
            result = {"message": "no method", "result": None}
        return result
    ...
    def get_none() -> str:
        ...
        # user defined method
        result = {"result": "api", "message": "running"}
        return jsonify(result)

    @staticmethod
    def get(m_method: str) -> str:
    ...
    def post(self, m_method: str) -> str:
        ...
        if model_id is None:
            ...
            result = {"message": "fail", "result": "model_id required"}
            return jsonify(result)
...
def set_route(app: Flask, node: ImagePub) -> None:
